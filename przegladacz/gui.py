...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Stężenia gazów")
...
class FilePreviewTable(QWidget):
    def __init__(self, parent):
        ...
        self.table_widget.setFont(QFont("Arial", 8))
        ...
        for i in range(self.spectrum_data_np.shape[0]):
            for j in range(self.spectrum_data_np.shape[1]):
                self.table_widget.setItem(
                    i, j, QTableWidgetItem(str(self.spectrum_data_np[i, j]))
                )

    def update_cells(self):
        for i in range(self.spectrum_data_np.shape[0]):
            for j in range(self.spectrum_data_np.shape[1]):
                self.table_widget.item(i, j).setText(str(self.spectrum_data_np[i, j]))
        return
